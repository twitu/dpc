...
-- node state consists of a clients init state
-- whether it is done and the server's default ready state
...
initNetwork = initializeNetwork nodeStates protlets
 where
  ...
  client   = 1
  nodeStates =
    [ (server, [(addLabel, ServerReady), (mulLabel, ServerReady)])
    , (client, [(addLabel, ClientInit server [1, 1])])
    ]
  protlets = [(addLabel, [compute sum]), (mulLabel, [compute product])]
...
addNetwork = initializeNetwork nodeStates protlets
 where
  nodeStates = [(0, [(0, ServerReady)]), (1, [(0, ClientInit 0 [3, 20])])]
  protlets   = [(0, [compute sum])]
...
mulServer
  :: (ProtletAnnotations S m, MessagePassing m, NetworkNode m) => Label -> m a
...
polynomialServer
  :: (ProtletAnnotations S m, MessagePassing m, NetworkNode m)
  => Label
  -> Label
  -> m a
 ...
 where
  ...
  loop = do
    enactingServer (OneOf [compute product, compute sum]) $ do
      Message client _ args _ msgLabel <- spinReceive
        [(addLabel, "compute__Request"), (mulLabel, "compute__Request")]
      let response = if
            | msgLabel == addLabel -> sum args
            | msgLabel == mulLabel -> product args
      send client msgLabel "compute__Response" [response]
    loop

parPolynomialServer
  :: (ProtletAnnotations S m, MessagePassing m, NetworkNode m, Par m)
  => Label
  -> Label
  -> m a
...
polynomialClient
  :: (ProtletAnnotations S m, MessagePassing m)
  => Label
  -> Label
  -> NodeID
  -> Arith
 ...
 where
  ...
  go (l :+: r   ) = do
    ...
    [ans] <- enactingClient (compute sum)
      $ rpcCall addLabel "compute" [l', r'] server
    return ans
  go (l :*: r) = do
    l'    <- go l
    r'    <- go r
    [ans] <- enactingClient (compute product)
      $ rpcCall mulLabel "compute" [l', r'] server
    return ans
...
addClient label a b server = do
  [ans] <- enactingClient (compute sum) $ rpcCall label "compute" [a, b] server
  return ans
...
addConf = initializeImplNetwork [(1, addClient 0 3 20 0), (0, addServer 0)]
