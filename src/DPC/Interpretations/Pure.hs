...
instance (Show a, Show s) => Show (DiSeL s a) where
  ...
  show (Send nodeid label tag body k) = concat
    [ "Send["
    , show label
    , ", "
    , tag
  ...
  show (Receive candidates _) =
    concat ["Receive[", show candidates, "] <Continuation>"]
...
stepDiSeL
  :: NodeID -> [Message] -> DiSeL s a -> (TraceAction s, [Message], DiSeL s a)
stepDiSeL nodeID soup (Pure a) = (InternalAction nodeID, soup, Pure a)
stepDiSeL nodeID soup (Send to label tag body k) =
  let msg = Message nodeID tag body to label
  in  (SendAction nodeID msg, msg : soup, k)
stepDiSeL nodeID soup (Receive candidates k) =
 ...
 where
  ...
  findMessageAndTransitionInSoup (m : soup') = case findInCandidates m of
    Nothing -> second ((:) m) <$> findMessageAndTransitionInSoup soup'
    Just _  -> Just (m, soup')
  findInCandidates Message {..} = find
    (\(lbl, tag) -> _msgTo == nodeID && _msgLabel == lbl && _msgTag == tag)
    candidates
stepDiSeL nodeID soup (This k   ) = (InternalAction nodeID, soup, k nodeID)
stepDiSeL nodeID soup (Par mas k) = case check mas of
  Nothing ->
    let ((n, ma) : mas') = mas
    in  let (act, soup', ma') = stepDiSeL nodeID soup ma
        in  (act, soup', Par (snoc mas' (n, ma')) k)
  Just as ->
 ...
 where
  ...
  check ((n, ma) : mas') = case ma of
    ...
    _      -> Nothing
stepDiSeL nodeID soup (Bind ma fb) = case ma of
  Pure a -> (InternalAction nodeID, soup, fb a)
  _ ->
    let (act, soup', ma') = stepDiSeL nodeID soup ma
    in  (act, soup', Bind ma' fb)
stepDiSeL nodeID soup (EnactingServer p ma) = case ma of
  Pure a -> (InternalAction nodeID, soup, Pure a)
  _ ->
    let (act, soup', ma') = stepDiSeL nodeID soup ma
    in  (ServerAction p act, soup', EnactingServer p ma')
stepDiSeL nodeID soup (EnactingClient p ma) = case ma of
  Pure a -> (InternalAction nodeID, soup, Pure a)
  _ ->
    let (act, soup', ma') = stepDiSeL nodeID soup ma
    in  (ClientAction p act, soup', EnactingClient p ma')
...
runPure initConf = go (cycle $ nodes initConf) initConf
 where
  go
    :: [NodeID]
    -> ImplNetwork (DiSeL s) a
    -> [(TraceAction s, ImplNetwork (DiSeL s) a)]
  go []       _    = []
  go (n : ns) conf = do
    let (act, soup', node') =
          stepDiSeL n (_globalState conf) (_localStates conf Map.! n)
...
type ValidationM s a
  = StateT (Map NodeID (ValidationState s)) (Either ValidationError) a

checkTrace
  :: Show s => SpecNetwork [] s -> [TraceAction s] -> Either ValidationError ()
checkTrace initNetwork trace = case runStateT (go trace) (Init <$> initConf) of
  Left  e       -> Left e
  Right ((), _) -> Right ()
 where
  initConf = Map.fromList
    [ (k, s)
    | (k, (v, _, _)) <- Map.assocs $ _localStates initNetwork
    , Running s      <- [v Map.! 0]
    ]
  ...
  checkAction (ServerAction (RPC pName _ serverStep) (ReceiveAction nodeID msg))
    = do
      vs <- fromJust <$> use (at nodeID)
      case vs of
        Init s -> case serverStep (_msgBody msg) s of
          Nothing ->
            fail
              . concat
              $ ["Expecting node ", show nodeID, " to serve ", pName]
          Just (resp, s') ->
  ...
  checkAction (ServerAction (RPC pName _ _) (SendAction nodeID msg)) = do
    ...
    case vs of
      SyncServerFor _ client resp s' ->
        if (_msgTo msg == client)
           && (_msgBody msg == resp)
           && (_msgTag msg == (pName ++ "__Response"))
        then
          at nodeID ?= Init s'
        else
          fail
          . concat
          $ [ "The server response did not follow the protocol from state: "
            , show s'
            , "\n"
            , "\tExpected: "
            , show resp
  ...
  checkAction (ClientAction (RPC pName clientStep _) (SendAction nodeID msg)) =
    do
      vs <- fromJust <$> use (at nodeID)
      case vs of
        Init s -> case clientStep s of
          Nothing ->
            fail
              . concat
              $ [ "Node expected to initiate rpc "
                , pName
                , "\nNode is in state: "
                , show s
                ]
          Just (server, body, k) ->
            if _msgBody msg == body && _msgTo msg == server
              then at nodeID ?= AwaitingResponseFrom pName server k
                ...
                fail
                . concat
                $ [ "Inappropriate request initiating rpc "
                  , pName
                  , "\nExpected:"
                  , show body
                  , " to "
      ...
      AwaitingResponseFrom pName' server k | pName == pName' ->
        if _msgFrom msg == server && _msgTag msg == pName ++ "__Response"
          then at nodeID ?= Init (k $ _msgBody msg)
            ...
            fail
            . concat
            $ ["Expected reception of response at client in RPC", pName]
      AwaitingResponseFrom pName' _ _ | pName == pName' ->
        fail
          . concat
          $ [ "Expected reception on protlet"
            , pName'
            , "but got message tagged "
            , _msgTag msg
            ]

  checkAction (ClientAction (ARPC pName clientStep _ _) (SendAction nodeID msg))
    = do
      vs <- fromJust <$> use (at nodeID)
      case vs of
        Init s -> case clientStep s of
          Nothing ->
            fail
              . concat
              $ [ "Node expected to initiate ARPC "
                , pName
                , "\nNode is in state: "
                , show s
                ]
          Just (server, body, k) ->
            if _msgBody msg == body && _msgTo msg == server
              then at nodeID ?= AwaitingResponseFrom pName server k
                ...
                fail
                . concat
                $ [ "Inappropriate request initiating ARPC "
                  , pName
                  , "\n\tExpected:"
                  , show body
                  , " to "
      ...
      AwaitingResponseFrom pName' server k | pName == pName' ->
        if _msgFrom msg == server && _msgTag msg == pName ++ "__Response"
          then at nodeID ?= Init (k $ _msgBody msg)
            ...
            fail
            . concat
            $ ["Expected reception of response at client in ARPC", pName]
      AwaitingResponseFrom pName' _ _ | pName == pName' ->
        fail
          . concat
          $ [ "Expected reception on protlet"
  ...
  checkAction (ServerAction (ARPC pName _ receiveStep _) (ReceiveAction nodeID msg))
    = do
      vs <- fromJust <$> use (at nodeID)
      case vs of
        Init s -> case receiveStep msg s of
          Nothing ->
            fail
              . concat
              $ ["Expecting node ", show nodeID, " to serve ARPC ", pName]
          Just s' -> at nodeID ?= AsyncServerFor pName (_msgFrom msg) s'
  checkAction (ServerAction (ARPC pName _ _ sendStep) (SendAction nodeID msg))
    = do
      vs <- fromJust <$> use (at nodeID)
      case vs of
        AsyncServerFor _ client s -> case sendStep nodeID s of
          (resp, s') : _ ->
            if (_msgTo msg == client)
               && (_msgBody msg == _msgBody resp)
               && (_msgTag msg == (pName ++ "__Response"))
            then
              at nodeID ?= Init s'
            else
  ...
  checkAction (ClientAction (Quorum pName _ broadcast _ _) (SendAction nodeID msg))
    = do
      vs <- stateOfNode nodeID
      case vs of
        Init s -> error "Unimplemented broadcast, first send action"
        Broadcasting s ->
