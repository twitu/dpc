...
-- Two node states
-- per node state consists of a protlet state for each protocol instance
-- and a message queue
...
instance Show s => Show (NodeState s) where
  ...
  show (BlockingOn s rpc f from _) = unwords
    [ "Blocking in state"
    , show s
    , "expecting"
    , show f
...
-- a protocol + droplet = protlet i.e. a unique pattern of interaction between
-- one of more nodes that can be composed to create bigger patterns
-- s is the data type of the state and f encapsulates the non-determinism e.g.
-- a list
data Protlet f s =
...
-- client step uses the client state [s] to send args [Int] to a server [NodeID]
-- and the response payload [Int] is wrapped and converted into the next client state
-- the conversion is done by taking a callback function [Int] -> s
-- Maybe because client can fail
type ClientStep s = s -> Maybe (NodeID, [Int], [Int] -> s)
-- server step uses arguments [Int] and server state
-- and produces the subsequent server state [s] and response
-- Maybe because server can fail
type ServerStep s = [Int] -> s -> Maybe ([Int], s)

-- receive a message and change state
type Receive s = Message -> s -> Maybe s
-- send a message to a target node and take the current state
-- return reponse message and the next state
type Send f s = NodeID -> s -> f (Message, s)

type Broadcast s = s -> Maybe ([(NodeID, [Int])], [(NodeID, [Int])] -> s)

-- Takes `f` as form of indeterminism like list, `s` as the data structure
-- representing state of nodes.
type SpecNetwork f s
  = NetworkState
      (Map Label [Protlet f s])
      (Map Label (NodeState s), [Message], Bool)

protletInstances :: SpecNetwork f s -> [Label]
protletInstances = Map.keys . _globalState

-- Initialize a network with an association of nodes to protlet instance states
-- and an association of protlet instsnaces to protlets.
initializeNetwork
  :: [(NodeID, [(Label, s)])] -> [(Label, [Protlet f s])] -> SpecNetwork f s
initializeNetwork ns protlets = NetworkState
  { _localStates = Map.fromList
                     [ (n, (nodestates states, [], True)) | (n, states) <- ns ]
  , _globalState = Map.fromList protlets
  }
 where
  -- labels are used to separate protlet states
  -- all states are wrapped with Running state for the node
...
applyTransition (Crash nodeID) = \network@NetworkState {..} ->
  network { _localStates = Map.adjust (_3 .~ False) nodeID _localStates }

-- update state of a protlet in a node
update
  :: Label
  -> NodeID
  -> NodeState s
  -> [Message]
  -> (SpecNetwork f s -> SpecNetwork f s)
update label nodeID s' inbox' network@NetworkState {..} = network
  { _localStates = Map.adjust
                     (\(ss, _, status) ->
                       (Map.insert label s' ss, inbox', status)
                     )
                     nodeID
                     _localStates
  }
...
deliver msg@Message {..} network@NetworkState {..} = network
  { _localStates = Map.adjust
                     (\(ss, inbox, status) -> (ss, msg : inbox, status))
                     _msgTo
                     _localStates
  }
...
resolveBlock label tag responsesNeeded nodeID inbox responders k = do
  ...
  pure $ ReceivedMessages label
                          nodeID
                          responses
                          (Running $ k (arrange <$> responses))
                          inbox'
 where
  findResponses
    :: (Monad m, Alternative m)
  ...
  isResponseFrom from Message {..} =
    _msgFrom == from && _msgTag == tag && _msgLabel == label
...
tryClientStep label protlet step nodeID state inbox = case step state of
  Just (server, req, k) -> pure $ SentMessages
    label
    nodeID
    [buildRequest server req]
    (BlockingOn state (protlet ++ "__Response") 1 [server] (k . snd . head))
    inbox
  _ -> empty
 where
  buildRequest receiver body = Message { _msgTag   = protlet ++ "__Request"
                                       , _msgTo    = receiver
                                       , _msgBody  = body
                                       , _msgFrom  = nodeID
                                       , _msgLabel = label
...
tryServerStep label protlet step nodeID state inbox = do
  ...
  case step _msgBody state of
    Just (ans, state') -> pure $ SentMessages label
                                              nodeID
                                              [buildReply _msgFrom ans]
                                              (Running state')
                                              inbox'
    _ -> empty
 where
  isRequest Message {..} =
    _msgTag == protlet ++ "__Request" && _msgLabel == label

  buildReply :: NodeID -> [Int] -> Message
  buildReply receiver body = Message { _msgTag   = protlet ++ "__Response"
                                     , _msgFrom  = nodeID
                                     , _msgBody  = body
                                     , _msgTo    = receiver
                                     , _msgLabel = label
...
tryBroadcast label name broadcast nodeID state inbox = case broadcast state of
  Just (casts, k) -> pure $ SentMessages
    label
    nodeID
    (buildRequest <$> casts)
    (BlockingOn state
                (name ++ "__Response")
                (fromIntegral $ length casts)
                (fst <$> casts)
                k
    )
    inbox
  _ -> empty
 where
  buildRequest (receiver, body) = Message { _msgTag   = name ++ "__Broadcast"
                                          , _msgTo    = receiver
                                          , _msgBody  = body
                                          , _msgFrom  = nodeID
                                          , _msgLabel = label
...
tryQuorum label name quorumSize broadcast nodeID state inbox =
  case broadcast state of
    Just (casts, k) -> pure $ SentMessages
      label
      nodeID
      (buildRequest <$> casts)
      (BlockingOn state (name ++ "__Response") quorumSize (fst <$> casts) k)
      inbox
    Nothing -> empty
 where
  buildRequest (receiver, body) = Message { _msgTag   = name ++ "__Quorum"
                                          , _msgTo    = receiver
                                          , _msgBody  = body
                                          , _msgFrom  = nodeID
                                          , _msgLabel = label
...
possibleTransitions
  :: (Monad f, Alternative f) => SpecNetwork f s -> f (Transition s)
...
possibleCrashes ns@NetworkState {..} = do
  ...
  if isOnline then return $ Crash nodeID else empty

-- Non-deterministically advance a network, "chosing" amongst possible transitions.
stepNetwork
  :: (Monad f, Alternative f) => SpecNetwork f s -> f (SpecNetwork f s)
stepNetwork network =
  applyTransition <$> possibleTransitions network <*> pure network
...
simulateNetworkTraces = go
  where go n = concatMap (n :) <$> (simulateNetworkTraces <$> stepNetwork n)
